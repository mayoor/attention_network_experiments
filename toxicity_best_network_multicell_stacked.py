from keras.layers import Bidirectional, Input, LSTM, Dense, Activation, Conv1D,RNN, Flatten, Embedding, MaxPooling1D, Dropout, RNN, LSTMCell
...
TRAIN_FILE_PATH = 'new_train.csv'#'/data/train.csv'
VAL_FILE_PATH = 'new_test.csv'
...
MODEL_SAVE_PATH = 'models/best_model_new_stacked.h5' #'/output/best_model.h5'
...
def main():
	...
	#train.head()
	train = df
	test = tu.clean_up(pd.read_csv(VAL_FILE_PATH))
...
This returns LSTM based model. There are 6 output classes, all soft sharing the parameters of a common network.
"""
def get_model_soft_sharing_lstm_singleoutput(emb_matrix, time_steps, learning_rate=0.001, n_classes=1, decay=0.1, layers=3):
