...
from keras.layers import Add, Concatenate, Lambda, Reshape, Permute, Average, Layer, TimeDistributed, Multiply, GRU
...
OUTPUT_FILENAME = 'sub_h_n_consolidated.csv'
SENTENCE = 4
WORDS = 80
...
def get_word_attention(emb_matrix, word_length, optimizer, nclasses, gru_output_dim=50):
    ...
    x = Bidirectional(GRU(gru_output_dim,return_sequences=True))(sequence_input)
...
def get_model_soft_sharing_lstm_singleoutput(emb_matrix, sentence_length, word_length, learning_rate=0.001, n_classes=1, decay=0.1, combined_model=False):
    ...
    rmsprop = optimizers.RMSprop(lr=learning_rate, clipnorm=0.1, clipvalue=0.05,decay=0.1)#
