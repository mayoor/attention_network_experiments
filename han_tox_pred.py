...
from keras.layers import Add, Concatenate, Lambda, Reshape, Permute, Average, Layer, TimeDistributed, Multiply, GRU, BatchNormalization, CuDNNGRU
...
#import han_classifier_tox_bn as hn
#nohup python -u han_classifier_tox_bn.py > class_output.log &
...
MODEL_SAVE_PATH = 'models/best_model_new_reg_2_990931.h5'#'models/best_model_new_reg_2_990145.h5'#'models/best_model_new_reg_2_988670.h5' #'/output/best_model.h5'
OUTPUT_FILENAME = 'sub_h_n_consolidated_filtered_inter.csv'
SENTENCE = 300
WORDS = 1
...
def get_sentence_attention(word_model , word_length, sent_length, n_classes):
    ...
    for i in range(nclasses):
        ...
        attn_final_sent = Dense(64, activation='relu')(attn_final_sent)
    ...
    for i in range(1,nclasses):
        x = Dense(32, activation='relu')(attn_sents_for_all_classes[i])
        x = Dropout(0.1)(x)
        p = Dense(1, activation='sigmoid')(x)
        preds.append(p)
...
model = load_model(MODEL_SAVE_PATH, custom_objects={"ATTNWORD": ATTNWORD})
